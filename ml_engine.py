...
try:
    ...
    print(f"✅ ML model loaded — {metadata['n_diseases']} diseases, {metadata['accuracy']}% accuracy")
...
def run_ml_diagnosis(symptom_list: list[str]) -> dict:
    """Run the local ML model to predict diseases."""
    if ml_model is None or label_encoder is None or not symptom_list:
        return {"error": "No symptoms or model not loaded", "top_conditions": []}

    vector = symptoms_to_vector(symptom_list)
    probas = ml_model.predict_proba(vector)[0]
    ...
    for idx in top_indices:
        ...
        top_conditions.append({
            ...
            "reasoning": f"ML model: {round(float(score) * 100, 1)}% probability",
        })

    return {"extracted_symptoms": symptom_list, "top_conditions": top_conditions}
...
# Complaint keywords → relevant body areas
_KEYWORD_BODY_MAP = {
...
# Complaint keywords → relevant lifestyle factors
...
# Complaint keywords → relevant pre-existing condition checks
...
# Complaint keywords → relevant family history checks
_KEYWORD_FAMILY_MAP = {
    "heart": ["Heart Disease", "Hypertension"],
    "chest": ["Heart Disease"],
    "sugar": ["Diabetes"], "thirst": ["Diabetes"],
...
def generate_dynamic_steps(complaint: str) -> list[dict]:
    ...
    # Only show the top relevant categories (max 5) — not all 10
